...
return res.json({
  ...
  data: {
    embeds: [{
      color: 0x3a3b40,
      description: "Use /about to learn the commands."
    }]
  }
});
```
  ...
  if (diff < cooldown) {
    ...
    return res.json({
      ...
      data: {
        embeds: [{
          color: 0xff4444,
          description: `Come back in ${formatTime(cooldown - diff)}`
        }]
      }
    });
...
for (let i = 0; i < top.length; i++) {
  ...
  rows += `${i + 1}. <@${top[i].userId}> - Coins ${top[i].balance.toLocaleString()}\n`;
